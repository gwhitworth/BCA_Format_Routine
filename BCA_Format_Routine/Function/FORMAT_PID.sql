...
BEGIN
   ...
   IF @p_pid IS NOT NULL
      SET @Rtn = SUBSTRING(LTRIM(RTRIM(CAST(@p_pid AS VARCHAR(50)))),1,3) + '-' +
				 SUBSTRING(LTRIM(RTRIM(CAST(@p_pid AS VARCHAR(50)))),4,3) + '-' +
				 SUBSTRING(LTRIM(RTRIM(CAST(@p_pid AS VARCHAR(50)))),7,3)
